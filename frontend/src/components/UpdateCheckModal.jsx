    ...
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/60 p-4" role="dialog" aria-modal="true">
      <div className="max-h-[85vh] w-full max-w-lg overflow-hidden rounded-xl border border-white/10 bg-cs2-bg-card shadow-2xl">
        <div className="border-b border-white/10 px-4 py-3">
          <h2 className="text-sm font-bold text-white">{title || "更新"}</h2>
          {info.latest_version ? (
            <p className="mt-1 font-mono text-[11px] text-zinc-400">
              最新版本 <span className="text-cs2-orange">{info.latest_version}</span>
              {info.current_version ? (
                <>
                  {" "}
                  · 当前 <span className="text-zinc-300">{info.current_version}</span>
                </>
              ) : null}
            </p>
          ) : null}
        ...
        <div className="max-h-[45vh] overflow-y-auto px-4 py-3">
          {err ? (
            ...
            <p className="text-sm text-zinc-300">已安装为最新版本。</p>
          ) : null}
          {!err && notes ? (
          ...
          {!err && !notes && !upToDate ? <p className="text-[12px] text-zinc-500">无发布说明</p> : null}
        </div>
