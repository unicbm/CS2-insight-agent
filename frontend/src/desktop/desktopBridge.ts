...
type OpenDialogProperty = "openDirectory" | "multiSelections" | "openFile";

interface OpenDialogFilter {
  name: string;
  extensions: string[];
}
...
export interface DesktopBridge {
  ...
  chooseDemoFiles(): Promise<string[]>;
  chooseDirectory(defaultPath?: string, title?: string): Promise<string>;
...
function createDesktopBridge(): DesktopBridge | null {
  ...
  return {
    ...
    async showOpenDialog(options = {}) {
      ...
      const filePaths = normalizeSelection(selected);
