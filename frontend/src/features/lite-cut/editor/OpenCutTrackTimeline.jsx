...
export default function OpenCutTrackTimeline({ body, onDropMedia }) {
  ...
  const [killAxisVisible, setKillAxisVisible] = useState(true);
  ...
  const rows = useMemo(() => {
    ...
    return [
      ...overlayTracks.map((track, index) => ({
        id: track.id,
        type: "overlay",
        label: track.label || t("liteCut.track.overlayDefault", { index: index + 1 }),
      ...
      ...tracks.map((track) => ({
        ...
        type: track.type,
        label: track.name || t(
          track.type === "video" ? "liteCut.track.videoLabel" : "liteCut.track.audioLabel",
  ...
  }, [body, selectedClipId, selectedTrackId, t]);
  const rowsById = useMemo(() => new Map(rows.map((row) => [String(row.id), row])), [rows]);
  // 击杀轴：录制时写入成片素材的击杀点，按片段当前的裁剪 / 位置 / 变速换算，只读。
  const killAxisItems = useMemo(() => collectKillAxisItems(body), [body]);
  const killAxisBands = useMemo(() => {
    const bands = [];
    for (const track of body?.tracks || []) {
      if (track?.type !== "video" || track?.hidden) continue;
      for (const clip of track.clips || []) {
        if (!readClipKillMarkers(clip).length) continue;
        const { start, end } = clipKillAxisSpan(clip);
        if (end > start) bands.push({ id: String(clip.id), start, end });
      }
    }
    return bands;
  }, [body]);
  const visibleKillAxisItems = useMemo(() => assignKillAxisLevels(
    killAxisItems.filter((item) => item.timelineSec >= visibleRange.start && item.timelineSec <= visibleRange.end),
    pixelsPerSecond,
  ), [killAxisItems, pixelsPerSecond, visibleRange]);
  const killAxisShown = killAxisVisible && killAxisItems.length > 0;
  const killAxisHeight = Math.min(58, 26 + visibleKillAxisItems.reduce((max, item) => Math.max(max, item.level), 0) * 7);
  const playheadSnapPoints = useMemo(() => {
    ...
    if (killAxisShown) for (const item of killAxisItems) points.push(item.timelineSec);
    return [...new Set(points.map((point) => point.toFixed(3)))].map(Number).sort((a, b) => a - b);
  }, [body?.markers, killAxisItems, killAxisShown, rows]);
  ...
  const laneHeight = rows.reduce((sum, row) => sum + row.height + 2, 0)
    + autoOverlayDropHeight + autoVideoDropHeight + (killAxisShown ? killAxisHeight + 2 : 0);
  const ticks = useMemo(() => {
  ...
  const editMarker = (marker) => {
    const label = window.prompt(t("liteCut.marker.promptName"), String(marker.label || ""));
    if (label == null) return;
    const color = window.prompt(t("liteCut.marker.promptColor"), String(marker.color || "#f59e0b"));
  ...
  return (
    <section className="flex h-full min-h-0 flex-col overflow-hidden border-t border-cs2-border bg-cs2-bg-sidebar">
      <div className="flex shrink-0 items-center gap-1 border-b border-cs2-border bg-cs2-bg-card px-2.5 py-1.5 shadow-sm">
        ...
        <ToolButton title={t("liteCut.timeline.deleteRightSide")} label={t("liteCut.timeline.deleteRight")} onClick={() => deleteTimelineSide("right")}><PanelRightClose className="h-3.5 w-3.5" /></ToolButton>
        <ToolButton title={t("liteCut.timeline.snapTitle")} label={t("liteCut.timeline.snap")} active={snapEnabled} onClick={toggleSnap}><Magnet className="h-3.5 w-3.5" /></ToolButton>
        <ToolButton
          title={t(killAxisItems.length ? "liteCut.killAxis.toggle" : "liteCut.killAxis.empty")}
          label={t("liteCut.killAxis.name")}
          active={killAxisShown}
          disabled={!killAxisItems.length}
          onClick={() => setKillAxisVisible((visible) => !visible)}
        ...
        <div className="ml-auto flex items-center gap-1">
          ...
          />
          <button type="button" onDoubleClick={() => applyTimelineZoom(1)} className="w-9 text-center font-mono text-[10px] text-cs2-text-muted hover:text-cs2-text-primary" title={t("liteCut.timeline.zoomReset")}>{Math.round(timelineZoom * 100)}%</button>
          </div>
          <ToolButton title={t("liteCut.timeline.zoomIn")} onClick={() => applyTimelineZoom(timelineZoom * 1.25)}><ZoomIn className="h-3.5 w-3.5" /></ToolButton>
        </div>
      </div>
            ...
            <div className="relative" data-oc-ruler>
              ...
              {markerItems.map(({ marker, time, level }) => (
                <button
                  ...
                  title={t("liteCut.marker.tooltip", {
                    label: marker.label || t("liteCut.marker.untitled"),
                  ...
                  onContextMenu={(event) => {
                    event.preventDefault();
                    event.stopPropagation();
                    if (window.confirm(t("liteCut.marker.deleteConfirm", { label: marker.label || formatTime(time) }))) useLiteCutTimelineStore.getState().deleteMarker(marker.id);
                  }}
          ...
          <div className="relative" style={{ minHeight: laneHeight }}>
            {killAxisShown ? <div data-oc-kill-axis className="grid border-b border-white/[0.06]" style={{ gridTemplateColumns: `${TRACK_HEADER_WIDTH}px ${timelineWidth}px`, height: killAxisHeight + 2 }}>
              <div
                ...
                title={t("liteCut.killAxis.laneHint")}
              >
                <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded border border-orange-400/35 bg-orange-500/10 text-orange-300"><Crosshair className="h-3 w-3" /></span>
                <span className="min-w-0 flex-1 truncate text-[10px] font-bold text-cs2-text-secondary">{t("liteCut.killAxis.name")}</span>
                <span className="shrink-0 font-mono text-[9px] text-cs2-text-muted">{killAxisItems.length}</span>
              </div>
              <div className="relative border-l border-cs2-border-subtle bg-cs2-bg-sidebar" style={{ height: killAxisHeight }}>
                {killAxisBands.map((band) => (
                  <div
                    key={band.id}
                    className="pointer-events-none absolute inset-y-0 border-x border-orange-400/15 bg-orange-500/[0.05]"
                    style={{ left: band.start * pixelsPerSecond, width: Math.max(0, band.end - band.start) * pixelsPerSecond }}
                  />
                ))}
                {visibleKillAxisItems.map((item) => {
                  const tone = KILL_AXIS_TONES[item.marker.kind] || KILL_AXIS_TONES.kill;
                  const isVictimView = item.marker.perspective === "victim";
                  return (
                    <button
                      key={item.id}
                      type="button"
                      data-timeline-kill-marker={item.id}
                      title={t("liteCut.killAxis.markerTooltip", {
                        label: killMarkerLabel(item.marker, t),
                        time: formatTime(item.timelineSec),
                      })}
                      onClick={() => setPlayhead(item.timelineSec)}
                      className="absolute z-[25] h-2.5 w-2.5 -translate-x-1/2 rounded-sm border border-black/40 transition-transform hover:scale-125"
                      style={{
                        left: item.timelineSec * pixelsPerSecond,
                        top: 8 + item.level * 7,
                        backgroundColor: tone.fill,
                        // 受害者 POV 段是同一次击杀的另一面，压暗以免看成两次击杀。
                        opacity: isVictimView ? 0.45 : 1,
                        boxShadow: `0 0 6px ${tone.ring}`,
                        clipPath: "polygon(50% 0, 100% 50%, 50% 100%, 0 50%)",
                      }}
                    />
                  );
                })}
                <div className="pointer-events-none absolute inset-y-0 z-20 w-px bg-cs2-accent/70" style={{ left: playheadSec * pixelsPerSecond }} />
              </div>
            </div> : null}
            {rows.map((row) => {
      ...
      {contextMenu ? (() => {
        ...
        const slipUnavailableReason = t(contextMenu.rowType === "overlay" || multiple
          ? "liteCut.timeline.menu.slipSelectionReason"
          : "liteCut.timeline.menu.slipRoomReason");
        const keyframeLabel = t(contextMenu.rowType === "audio"
          ? "liteCut.timeline.menu.addAudioKeyframe"
          : "liteCut.timeline.menu.addTransformKeyframe");
        return createPortal(<div
          ref={contextMenuRef}
          role="menu"
          aria-label={t("liteCut.timeline.menu.ariaLabel")}
        ...
        >
          <ContextMenuItem label={t("liteCut.timeline.menu.splitAtPlayhead")} shortcut="S" onClick={() => runContextAction(actions.splitAtPlayhead)} />
          <ContextMenuItem label={t("liteCut.timeline.menu.splitAllAtPlayhead")} shortcut="Shift+S" onClick={() => runContextAction(actions.splitAllAtPlayhead)} />
          <ContextMenuItem label={t("liteCut.timeline.menu.duplicate")} shortcut="Ctrl+D" onClick={() => runContextAction(actions.duplicateSelected)} />
          ...
          <ContextMenuItem label={t("liteCut.timeline.menu.unlink")} disabled={!canUnlink} reason={t("liteCut.timeline.menu.unlinkReason")} onClick={() => runContextAction(actions.unlinkSelectedClips)} />
          <ContextMenuItem label={t("liteCut.timeline.menu.slipBackward")} shortcut="," disabled={!canSlipBackward} reason={slipUnavailableReason} onClick={() => runContextAction(() => actions.slipSelectedFrame(-1))} />
          <ContextMenuItem label={t("liteCut.timeline.menu.slipForward")} shortcut="." disabled={!canSlipForward} reason={slipUnavailableReason} onClick={() => runContextAction(() => actions.slipSelectedFrame(1))} />
          <ContextMenuItem
            label={keyframeLabel}
            shortcut="Alt+K"
            disabled={keyframeDisabled}
            reason={t("liteCut.timeline.menu.keyframeSelectionReason")}
          ...
          />
          <ContextMenuItem label={t("liteCut.timeline.menu.addMarker")} shortcut="M" onClick={() => runContextAction(actions.addMarkerAtPlayhead)} />
          <ContextMenuItem label={t("liteCut.timeline.menu.selectLeft")} shortcut="Alt+Shift+←" onClick={() => runContextAction(() => actions.selectTimelineItemsRelativeToClip(contextMenu.clipId, "left"))} />
          <ContextMenuItem label={t("liteCut.timeline.menu.selectRight")} shortcut="Alt+Shift+→" onClick={() => runContextAction(() => actions.selectTimelineItemsRelativeToClip(contextMenu.clipId, "right"))} />
          <div className="my-1 border-t border-cs2-border" />
          <ContextMenuItem label={t("liteCut.timeline.menu.rippleDelete")} shortcut="Ctrl+Delete" disabled={!canRipple} reason={t("liteCut.timeline.menu.rippleDeleteReason")} danger onClick={() => runContextAction(actions.rippleDeleteSelected)} />
          <ContextMenuItem label={t("liteCut.timeline.menu.delete")} shortcut="Delete" danger onClick={() => runContextAction(actions.deleteSelected)} />
        </div>, document.body);
      })() : null}
      {shortcutHelpOpen ? <div role="dialog" aria-modal="true" aria-label={t("liteCut.timeline.help.title")} className="fixed inset-0 z-[110] flex items-center justify-center bg-black/60 p-4" onPointerDown={() => setShortcutHelpOpen(false)}>
        <div className="max-h-[80vh] w-full max-w-xl overflow-auto rounded-2xl border border-cs2-border bg-cs2-bg-elevated p-4 shadow-2xl" onPointerDown={(event) => event.stopPropagation()}>
          <div className="flex items-center justify-between">
            <div><h3 className="text-sm font-bold text-cs2-text-primary">{t("liteCut.timeline.help.title")}</h3><p className="mt-0.5 text-[10px] text-cs2-text-muted">{t("liteCut.timeline.help.subtitle")}</p></div>
            <button type="button" onClick={() => setShortcutHelpOpen(false)} className="rounded-md px-2 py-1 text-cs2-text-muted hover:bg-white/5 hover:text-white">{t("liteCut.timeline.help.close")}</button>
          </div>
          <div className="mt-4 grid grid-cols-2 gap-x-6 gap-y-2 text-[11px]">
            ...
            ].map(([key, shortcut]) => <Fragment key={key}><span className="text-cs2-text-secondary">{t(`liteCut.timeline.help.${key}`)}</span><kbd className="text-right font-mono text-cs2-accent">{shortcut}</kbd></Fragment>)}
          </div>
          <div className="mt-4 grid gap-2 border-t border-cs2-border pt-3 text-[10px] leading-relaxed text-cs2-text-muted">
            <p><span className="font-semibold text-cs2-text-primary">{t("liteCut.timeline.help.markerTerm")}</span>{t("liteCut.timeline.help.markerNote")}</p>
            <p><span className="font-semibold text-cs2-text-primary">{t("liteCut.timeline.help.keyframeTerm")}</span>{t("liteCut.timeline.help.keyframeNote")}</p>
            <p><span className="font-semibold text-cs2-text-primary">{t("liteCut.timeline.help.killAxisTerm")}</span>{t("liteCut.timeline.help.killAxisNote")}</p>
          </div>
        </div>
      </div> : null}
    </section>
