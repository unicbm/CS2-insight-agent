...
async def get_or_index_demo_roster(
    ...
    *,
    ...
    """Return the persisted roster, parsing the immutable Demo only on a cache miss."""

    cached = cached_rows if cached_rows is not None else await demo_db.list_demo_player_stats(demo_id)
    if cached:
        return {
            "players": _roster_rows_for_api(cached),
            "cache_hit": True,
            "indexed": True,
            "error": None,
        }
    ...
    async with lock:
        # Single-flight by demo: concurrent endpoints share the first parse.
        cached = await demo_db.list_demo_player_stats(demo_id)
        if cached:
            return {
                "players": _roster_rows_for_api(cached),
                "cache_hit": True,
